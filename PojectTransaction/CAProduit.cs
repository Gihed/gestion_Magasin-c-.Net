...
{
    public partial class CAProduit : Form
    {
        ...
        private void CAProduit_Load(object sender, EventArgs e)
        {
            MySqlCommand commandDB = new MySqlCommand("select p.nom, SUM(p.prix_unitaire*(1+p.tva)*t.qteavendre) AS Total from transaction t inner join produit p on t.codeproduit = p.code inner join client c on c.codec = t.codeclient GROUP BY p.code", databaseConnection);
            try
            {
